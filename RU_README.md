...
- Нет прямого вызова в `xray-core v26.6.27`

Примечания:

- Эта обертка сохраняет legacy-поведение chain-hash, которое существовало в старых версиях `xray-core`.
- В актуальных версиях `xray-core`, включая `v26.6.27`, upstream использует hash leaf-certificate вместо старого chain-hash helper.
...
- В `xray-core v26.6.27` leaf hash helper предоставляется как генерация hash для уже распарсенного сертификата, а не как PEM-specific helper.

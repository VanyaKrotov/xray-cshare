...
- None directly in `xray-core v26.6.27`

Notes:

- This wrapper preserves the legacy chain-hash behavior that existed in older `xray-core` versions.
- In current `xray-core` releases such as `v26.6.27`, upstream exposes leaf-certificate hashing instead of the old chain-hash helper.
...
- `tls.GenerateCertHashHex(certs[0])` after parsing PEM or DER certificate input

Notes:

- In `xray-core v26.6.27`, the leaf hash helper is exposed as certificate-hash generation on parsed certificate data rather than as a PEM-specific helper.
